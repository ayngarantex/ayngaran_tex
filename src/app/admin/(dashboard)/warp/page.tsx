...
import { fetchWarps, fetchWarpsCount, fetchWarpSummary, fetchWarpSummaryCount } from '@/app/api/node/warp';
...
export default async function Page(props: {
  ...
  // if (view === 'summary') {
  totalCount = await fetchWarpSummaryCount(query, loomId, sizingId);
  summary = await fetchWarpSummary(query, loomId, sizingId, currentPage);
  // } else {
  //   totalCount = await fetchWarpsCount(query, loomId, loomStatus, sizingId);
  //   warps = await fetchWarps(query, currentPage, loomId, loomStatus, sizingId);
  // }

  const totalPages = Math.ceil(Number(totalCount) / pageLimit);

  return (
    <div className="w-full">
      <div className="flex w-full items-center justify-between border-b border-gray-200 pb-4">
        <h1 className="text-2xl font-bold text-gray-800">Warps ({totalCount || 0})</h1>
