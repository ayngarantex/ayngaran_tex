...
export default function EditForm({
  ...
  useEffect(() => {
    if (purchase) {
      const supplier = suppliers.find(c => Number(c.SupplierId) === Number(purchase.SupplierId)) || null;
      ...
      const mappedPayments = (purchase.purchase_payment_details || []).map((row: any, rowIndex: number) => ({
        pId: rowIndex,
      ...
      mappedPayments.push({ pId: mappedPayments.length, date: "date", amount: "", type: "Bank", to: "" });
      setPaymentsList(mappedPayments);

      setBeforeTax(Number(purchase.BeforeTax) || 0);
      setTaxPercentage(Number(purchase.TaxPercentage) || 0);
      setCgstAmount(Number(purchase.Cgst) || 0);
      setSgstAmount(Number(purchase.Sgst) || 0);
      setIgstAmount(Number(purchase.Igst) || 0);
      setAfterTax(Number(purchase.AfterTax) || 0);
      setBillType(purchase.BillType || "gst");
      setRoundOff(purchase.RoundOff || "");
