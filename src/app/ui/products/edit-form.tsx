...
import { updateProduct } from '@/app/api/node/product';
...
export default function EditForm({
  ...
  // Pre-fill state from product data
  useEffect(() => {
    setProductName(product?.Name || "");
    setProductType(product?.Type || "");
    setHsnCode(product?.HSNCode || "");
    setImages(product?.Image ? product.Image.split(',').map((img: string) => img.trim()).filter(Boolean) : []);
  ...
  return (
    <form className="max-w-4xl">
      ...
      <div className="mt-8 flex justify-end gap-4">
        ...
        <Button type="button" onClick={handleSubmit}>Update Product</Button>
      </div>
    </form>
  );
}
