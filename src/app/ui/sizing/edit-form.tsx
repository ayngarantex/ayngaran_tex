...
import { updateSizing } from '@/app/api/node/sizing';
...
export default function EditForm({
  ...
  useEffect(() => {
    const supplier = suppliers.find(c => Number(c.SupplierId) === Number(sizing?.SupplierId)) || null;
    setSelectedSuppliers(supplier);
    const yarn = yarns.find((e: any) => e.YarnId === sizing?.YarnId) || null;
    setSelectedYarns(yarn)
    setYarnId(sizing?.YarnId)
    setSupplierId(sizing?.SupplierId);
    setInvoiceNumber(sizing?.InvoiceNumber);
    setInvoiceDate(formatDateToLocalNew(sizing?.InvoiceDate))
    setWarpType(sizing?.WarpType)
    setBeforeTax(sizing?.BeforeTax);
    setTaxPercentage(sizing?.TaxPercentage);
    setCgstAmount(sizing?.Cgst);
    setSgstAmount(sizing?.Sgst);
    setIgstAmount(sizing?.Igst);
    setAfterTax(sizing?.AfterTax);
    setBillType(sizing?.BillType);
    setRoundOff(sizing?.RoundOff);
    setColor(sizing?.Color)
    setMeters(sizing?.Meters)
    setYarnSent(sizing?.YarnSent)
    setYarnUsed(sizing?.YarnUsed)
    setYarnBalance(sizing?.YarnBalance)
    setPrice(sizing?.Price)

    setInvoiceAmount(sizing?.InvoiceAmount);
    setPaidAmount(sizing?.ReceivedAmount);

    setWarpDetails(sizing?.sizing_warp_details || []);
    setInvPayments(sizing?.payment_details || []);
  }, [sizing, suppliers]);
  ...
  const invoiceProductData = useMemo(() => {
    return sizing?.sizing_yarn_details ?? [];
  }, [sizing?.sizing_yarn_details]);

  const sizingWarapData = useMemo(() => {
    return sizing?.sizing_warp_details ?? [];
  }, [sizing?.sizing_warp_details]);

  const invoicePaymentData = useMemo(() => {
    return sizing?.sizing_payment_details ?? [];
  }, [sizing?.sizing_payment_details]);
  ...
  const handleSubmit = async () => {
    ...
    if (warpDetails?.length) {
      warpDetails.forEach((row: any) => {
        if (row.date || row.weight || row.meters || row.loomId) {
    ...
    <form>
          ...
          <div className='bg-blue-300 rounded-lg p-4 my-4'>
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
              ...
              <div className="mb-4 w-full">
                ...
                <div className="relative mt-2">{selectedYarns?.InvoiceDate ? formatDateNew(selectedYarns?.InvoiceDate) : ""}</div>
              </div>
            </div>
      ...
      <div className="mt-6 flex justify-end gap-4">
        ...
        <Button type="button" disabled={submitLoading} onClick={handleSubmit}>
          {submitLoading ? 'Saving...' : 'Update Invoice'}
        </Button>
      </div>
    </form>
  );
